...
每篇文章底部会自动展示两类关联（来源 notes / garden / articles）：

- **→ 相关笔记** — 通过 `related` frontmatter 手动关联，或标签重叠自动推荐
- **← 上游知识** — 其他文章中 `related` 引用了当前文章，自动聚合为上游知识

书写示例：在 A 的 frontmatter 写 `related: ["notes/redis"]`，A 页面出现 redis 作为"相关笔记"，redis 页面出现 A 作为"上游知识"。一条 `related` 双向生效。
